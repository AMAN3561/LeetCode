...
class Solution {
public: 
    bool isSafe(int new_x, int new_y, int old_color, vector<vector<int>>& ans){
        if(new_x >= 0 && new_y >= 0 && new_x < ans.size() && new_y < ans[0].size() && ans[new_x][new_y] == old_color){
            return true;
        }
        else{
            return false;
    ...
    void dfstraversal(vector<vector<int>>& image, vector<vector<int>>& ans, int old_color, int new_color, int sr, int sc){
        ans[sr][sc] = new_color;
        ...
        for(int i = 0; i<4; i++){
            int new_x = dx[i] + sr;
            int new_y = dy[i] + sc;
            if(isSafe(new_x, new_y, old_color, ans)){
                dfstraversal(image, ans, old_color, new_color, new_x, new_y);
            }
        }
    }
    vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc, int color) {
        ...
        // dfs traversal :
        dfstraversal(image, ans, old_color, new_color, sr, sc);
        return ans;
    }
};